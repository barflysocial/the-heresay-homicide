...
async function loadAll() {
  ...
  $('truthPackSelect').innerHTML = packs.map(p => `<option value="${p.id}" ${p.id === 'heresay-homicide-rookie' ? 'selected' : ''}>${escapeHtml(p.gameTitle || 'Heresay Homicide')} — ${escapeHtml(p.levelLabel || p.difficulty || p.title)}</option>`).join('');
...
async function createSession() {
  ...
  const truthPackId = $('truthPackSelect').value || 'heresay-homicide-rookie';
