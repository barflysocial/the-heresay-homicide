...
function createDatabase() {
  if (!process.env.DATABASE_URL) return makeJsonFallback();

  const primary = new PostgresDatabase(process.env.DATABASE_URL);
  let fallback = null;
  ...
  return {
    async init() {
      try {
        ...
        console.error('PostgreSQL init failed. Falling back to local JSON sessions so the app stays online.');
        console.error(err && err.message ? err.message : err);
        fallback = makeJsonFallback();
        await fallback.init();
        this.mode = 'local JSON file fallback';
      }
    },
    async listSessions() {
      return (fallback || primary).listSessions();
    },
    async saveSession(session) {
      return (fallback || primary).saveSession(session);
    },
    async deleteSession(code) {
      return (fallback || primary).deleteSession(code);
    },
    async close() {
      return (fallback || primary).close();
    }
  };
}
